...
proptest! {
    ...
    /// `is_withdraw > 0` means withdraw a fraction of the current balance.
    ///
    /// Because caps are unlimited (`0`) and we always mint before tipping,
    /// operations always succeed — no `catch_unwind` is needed.
    #[test]
    fn test_token_conservation_invariant(
        ...
        // Create creators.  Use a static list of short Symbols since
        // `format!` is unavailable under the crate-level #![no_std].
        let syms = [
            Symbol::new(&t.env, "a"),
            Symbol::new(&t.env, "b"),
            Symbol::new(&t.env, "c"),
        ...
        for (ci_raw, is_wd, amt) in &ops {
            ...
            if *is_wd == 0 {
                ...
                t.tip_client().tip(
                    &tipper, &creator, &t.token_id, amt, &s(&t.env, ""),
                );
            } else {
